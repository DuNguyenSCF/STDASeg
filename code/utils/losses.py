...
class ENT_prior_class_weights(nn.Module):
    ...
    def forward(self, pred, prob, label=None):
        ...
        for i in range(batch_size):
            ...
            if hist[1] == 0:
                weight[1] = 1/torch.pow(hist.sum(), 1-self.ratio)
            weights.append(weight)
            
        weights = torch.stack(weights, dim=0)
        
        ent_w_ = (prob*torch.log(prob+1e-6))*weights.unsqueeze(2).unsqueeze(3).to(prob.device)
        ent_w = -1*torch.sum(ent_w_, dim=(1)) / torch.tensor(np.log(self.num_class)).cuda()
        # torch.sum(out,dim=(1,2)).sum()/4

        return torch.sum(ent_w,dim=(1,2)).sum() / batch_size
    
ent_prior_class_w = ENT_prior_class_weights(num_class=2, ratio=0.2)

class IW_MaxSquareloss(nn.Module):
    def __init__(self, ignore_index= -1, num_class=2, ratio=0.2):
        super().__init__()
        self.ignore_index = ignore_index
        self.num_class = num_class
        self.ratio = ratio
    ...
    def forward(self, pred, prob, label=None):
        ...
        msl_w_ = (torch.pow(prob, 2))*weights.unsqueeze(2).unsqueeze(3).to(prob.device) # N x C x H x W
        msl_w = -1*torch.sum(msl_w_) / (batch_size*2)
        # torch.sum(out,dim=(1,2)).sum()/4
